...
import com.agarang.domain.record.dto.mapper.NormalFeedingMapper;
 ...
 * fileName       : NormalFeedingService.java<br>
 * author         : nature1216 <br>
 * date           : 1/26/25<br>
 * description    : 일반수유 serviec 클래스입니다. <br>
 * ===========================================================<br>
 * DATE              AUTHOR             NOTE<br>
 * -----------------------------------------------------------<br>
...
public class NormalFeedingService implements FeedingTypeService<NormalFeedingGetResponse, FeedingUpdateRequest> {
    private final NormalFeedingMapper normalFeedingMapper;
    private final NormalFeedingRepository normalFeedingRepository;

    private static final int NORMAL_FEEDING_DEFAULT_AMOUNT = 0;

    /**
     * 일반 수유 기록을 생성합니다.
     *
     * <p>
     * 주어진 수유 정보(`Feeding`)와 요청 데이터를 기반으로 일반 수유 기록을 생성하고 저장합니다.
     * 수유량을 계산하여 `NormalFeeding` 엔터티에 저장합니다.
     * </p>
     *
     * @param feeding 수유 기록과 연관된 기본 `Feeding` 엔터티
     * @param request 일반 수유 기록 생성 요청 객체
     */
    @Override
    public void createFeedingRecord(Feeding feeding, FeedingCreateRequest request) {
        int amount = getAmountValue(feeding, request.getAmount());
        
        NormalFeeding normalFeeding = NormalFeeding.builder()
                .amount(amount)
                .feeding(feeding)
                .build();

        normalFeedingRepository.save(normalFeeding);
    }
     ...
     * `NormalFeeding` 엔터티를 조회하고, 변환된 응답 객체를 반환합니다.
     * </p>
     *
     * @param record  조회할 기록 정보
     * @param feeding 조회할 수유 정보
     * @return 조회된 일반 수유 기록 정보를 포함한 {@link NormalFeedingGetResponse} 객체
     * @throws BusinessException 기록을 찾을 수 없는 경우 발생
     */
    @Override
    public NormalFeedingGetResponse getFeedingRecord(Record record, Feeding feeding) {
        NormalFeeding normalFeeding = normalFeedingRepository.findById(record.getRecordId())
                .orElseThrow(() -> new BusinessException(ErrorCode.RECORD_NOT_FOUND));

        return normalFeedingMapper.mapToGetResponse(record, feeding, normalFeeding);
    }

    /**
     * 특정 일반 수유 기록을 업데이트합니다.
     *
     * <p>
     * 주어진 기록 ID를 기반으로 `NormalFeeding` 엔터티를 조회한 후,
    ...
    public void updateFeedingRecord(Integer recordId, FeedingUpdateRequest request) {
        NormalFeeding normalFeeding = normalFeedingRepository.findById(recordId)
                .orElseThrow(() -> new BusinessException(ErrorCode.RECORD_NOT_FOUND));

        normalFeedingMapper.updateNormalFeedingFromNormalFeedingUpdateRequest(request, normalFeeding);
    }
     ...
     *
     * <p>
     * 주어진 기록 ID를 기반으로 `NormalFeeding` 엔터티를 조회한 후, 해당 기록을 삭제합니다.
     * </p>
     *
    ...
    public void deleteFeedingRecord(Integer recordId) {
        ...
        normalFeedingRepository.delete(normalFeeding);
    }

    /**
     * 일반 수유 유형을 반환합니다.
     *
     * @return 일반 수유 유형 {@link FeedingType#NORMAL_FEEDING}
     */
    @Override
    public FeedingType getFeedingType() {
        return FeedingType.NORMAL_FEEDING;
    }

    /**
     * 특정 날짜의 총 일반 수유량을 반환합니다.
     *
     * <p>
     ...
     */
    @Override
    public Integer getDailyTotalAmount(Baby baby, LocalDate date) {Double average = normalFeedingRepository
     ...
     * @return 최종 결정된 수유량
     */
    private Integer getAmountValue(Feeding feeding, Integer requestValue) {
        LocalDate end = LocalDate.now();
        LocalDate start = end.minusMonths(1);

        Integer latestValue = normalFeedingRepository
                .getLatestNormalFeedingByBabyAndStartedAtBetween(feeding.getRecord().getBaby(), start, end)
                .map(NormalFeeding::getAmount)
                .orElse(NORMAL_FEEDING_DEFAULT_AMOUNT);
